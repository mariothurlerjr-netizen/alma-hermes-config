...
§
Mario prefers operational command batches to be delivered as a single Claude Code prompt when he will run them himself; if Hermes can safely implement directly on the VPS, do it directly. Hermes should keep going until the task is complete and follow the next relevant step proactively.
§
