...
- Ao escrever métricas em Markdown, não colocar nomes crus de campanhas/contas com `|` dentro de tabelas sem escape. Campanhas como `ALMA Local | ET` quebram colunas e viram status ambíguo. Preferir lista (`- ALMA Local | ET: 28/dia`) ou escapar pipes (`ALMA Local \| ET`) antes de gerar a tabela.
...
- Na janela da manhã, aplicar a mesma disciplina de verificação: depois de sobrescrever `checkin-manha.md`, `status-instantly.md` e `heartbeat-state.md`, reler os três e `agents/shield/inbox.md`. Só considerar fechado se `morning_07_15_executed: yes` e `instantly_status_morning_executed: yes` estiverem na seção da data BRT atual, os arquivos carregarem o mesmo `collected_at` da coleta única, e a linha de desvio estiver presente no SHIELD quando aplicável. Mesmo após verificação positiva, a resposta final do default continua exatamente `[SILENT]`.
- Ao verificar se `agents/shield/inbox.md` ficou sem poluição de numeração persistida, não use o output bruto de `read_file` como fonte para regex `^\s*\d+\|`: a própria ferramenta adiciona prefixos `N|` em cada linha e pode gerar falso positivo. Para esse check específico, leia o arquivo cru via Python (`Path(path).read_text()`) ou outro método sem prefixo, e só então procure `^\s*\d+\|`.
- Antes de sobrescrever arquivos operacionais do heartbeat (`status-instantly.md`, `checkin-manha.md`, `checkin-noite.md`, `heartbeat-state.md`), ler o arquivo alvo quando ele existir e tratar `write_file` warnings de modificação por sibling/subagent como sinal para verificação pós-write, não como sucesso cego. Se a janela pede overwrite, a escrita continua válida, mas a validação precisa confirmar que o conteúdo final contém o `collected_at` da coleta única e os marcadores esperados.
- Em status Instantly, separar rigorosamente `campanhas ativas agora` de `envios realizados hoje`. A API de analytics pode reportar sends de campanhas que foram pausadas/desativadas depois do envio; não inferir cap ativo a partir de sends do dia. Escrever ambos quando divergirem, ex.: `campanhas ativas: 1 / cap live 208`, mas `sends hoje: V3 580, legacy 127, Local 128`, e registrar a divergência como sinal para SHIELD/ORCHESTRATOR se for material.
- Na janela da noite, compare explicitamente o snapshot da manhã com a coleta live da noite para mudanças materiais intradiárias em Instantly, especialmente `campanhas_ativas`, `cap ativo`, `replies`, `discoveries`, `seed gate`. Se a manhã tinha 1 campanha/cap baixo e a noite tem várias campanhas/cap maior ainda sem seed-test formal, tratar como desvio novo, sobrescrever status/check-in/state com a coleta única e appendar uma linha nova no SHIELD com o delta, não só repetir o desvio da manhã.
